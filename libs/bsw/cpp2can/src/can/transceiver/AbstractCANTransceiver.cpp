...
{
    ESR_UNUSED const SuspendResumeAllInterruptsScopedLock lock;
    _listeners.remove_if([&](const ICANFrameListener& _listener){return &listener == &_listener;});
}
...
{
    ESR_UNUSED const SuspendResumeAllInterruptsScopedLock lock;
    _sentListeners.remove_if([&](const IFilteredCANFrameSentListener& _listener){return &listener == &_listener;});
}
